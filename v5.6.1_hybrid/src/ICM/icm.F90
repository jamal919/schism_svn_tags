...
!--------------------------------------------------------------------------------
  ...
  use icm_mod, only : wqc,dep,Temp,Sal,TSED,ZB1,ZB2,PB1,PB2,PB3,RPOC,LPOC,DOC,RPON,LPON, &
                    ...
                    & iPh,TIC,ALK,CA,CACO3,PH,PH_el,Chl_el,CChl
  implicit none 
  ...
  elseif(imode==2) then
    ...
    do k=kbe(id)+1,nvrt
      ...
      Chl_el(k,id)=PB1(m,1)/CChl(1)+PB2(m,1)/CChl(2)+PB3(m,1)/CChl(3)
  ...
  if(rIa>30) then !photosynthesis critia
    ...
    do k=1,nv
      ...
      do i=1,3
        ...
        if(xT>0.0) then
          rval=rKTGP1(i)*xT*xT
          if(rval>50.d0.or.rKTGP1(i)<0) then
            write(errmsg,*)'check PB growth rKTGP1, xT: ',xT,rKTGP1,rval
            call parallel_abort(errmsg)
          endif
          GPT(i)=GPM(i)*exp(-rval)
          !GPT(i)=GPM(i)*exp(-rKTGP1(i)*xT*xT)
        else
          rval=rKTGP2(i)*xT*xT
          if(rval>50.d0.or.rKTGP2(i)<0) then
            write(errmsg,*)'check PB growth rKTGP2, xT: ',xT,rKTGP2,rval
            call parallel_abort(errmsg)
          endif
          GPT(i)=GPM(i)*exp(-rval)
          !GPT(i)=GPM(i)*exp(-rKTGP2(i)*xT*xT)
        endif
      enddo
      ...
      if(rKeh<0) then
        write(errmsg,*)'check ICM iLight rKeh: ',rKe,dep(k),rKeh
        call parallel_abort(errmsg)
      endif
      ...
      do i=1,3
        ...
        elseif(jLight==2) then !Cerco, 
          ...
          rIK=(1.d3*CChl(i))*GPT(i)/alpha_PB(i) 
...
!--------------------------------------------------------------------------------------
    ...
    if(id<=ne.and.iout_icm==1.and.mod(it,nspool_icm)==0) then
     ...
     & RPOC(k,1),rKRPOC,WSRP(id),FCRP,RPOC0,nRPOC,&
     & LPOC(k,1),rKLPOC,WSLP(id),FCLP,RPOC0,nLPOC,&
     & DOC(k,1),xKHR,xDenit,rKDOC,rKHORDO,rKDC,rKTDOM,FCDP,FCD(1),FCD(2),FCD(3),rKHR1,rKHR2,rKHR3,nDOC,&
