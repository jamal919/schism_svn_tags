...
!
!        Convert to Wave Action if necessary
!
      IF (LWBAC2EN) THEN
        DO IS = 1, NUMSIG
...
!
!        Convert from nautical to Cartesian direction if necessary and from deg2rad
!
      DO IS = 1, WBNUMSIG
...
!           WRITE (*,*)  ETAIL * EAD, EFTAIL * OMEG * EAD, WALOC(NUMSIG,ID)
      ...
      IF (.TRUE.) THEN ! Write WW3 spectra of the input boundary condition ...
...
!**********************************************************************
      ...
      END SUBROUTINE
!**********************************************************************
!* KM: impose depth-limited breaking at the boundary nodes            *
!* The objective is to avoid shadow areas, which induce large vortices*
!* The strategy consists in reducing the energy at nodes depending on *
!* the local depth to first approximation. Then the energy is corrected
!* depending on the surrounding nodes.
!* NB: This is a temporary solution prior to implement a 1-D refraction/breaking model.
!**********************************************************************
      SUBROUTINE SET_WAVE_BREAKING_BOUNDARY
      USE DATAPOOL
      USE SCHISM_GLBL, ONLY: nnp, indnd, isbnd
      IMPLICIT NONE
      INTEGER :: IP, eIdx, IS, ID, INN, INDN_1 = 0, INDN_2 = 0
      REAL(rkind) :: HS, HSBC, ETOT, SME01, SME10, KME01, KMWAM, KMWAM2
      REAL(rkind) :: BETA, EMAXLOC, BRHDLOC
      IF (LBCWA .OR. LBCSP) THEN ! Spectrum or parametric boundary condition
        DO IP = 1, IWBMNP
		  ! Index of boundary node
          eIdx = IWBNDLC(IP)
		  
		  ! Getting mean wave parameters from the imposed action spectrum
          CALL MEAN_WAVE_PARAMETER(IP,AC2(:,:,eIdx),HSBC,ETOT,SME01,SME10,KME01,KMWAM,KMWAM2)
		  BRHDLOC = HSBC*(DEP(eIdx) + eta1(eIdx))
		  
		  IF (BRHDLOC.LE.BRHD.OR.DEP(eIdx).LE.ZERO) THEN
		    CONTINUE
          ELSE		  
		    ! Local maximum of energy allowed for that depth
		    EMAXLOC = (BRHD/sqrt(2.d0)*DEP(eIdx))**2/8
		  
		    ! Computing the ratio between the max energy and what is imposed
		    BETA = EMAXLOC/ETOT
		  
		    ! We adjust the wave action spectrum, starting from the principle that the saturation concept will always dissipate too much energy
			IF (BETA.GT.1) THEN
			  CONTINUE
            ELSE
			  ! We impose the reduction to the wave action, based on the 'saturation' principle
		      AC2(:,:,eIdx) = BETA*AC2(:,:,eIdx)
			  
			  ! We adjust to the surrounding nodes (internal only)
			  ! We're only interested in boundary nodes with two interior neighbors 
			  ! We first look for the index of these two neighbors
			  !DO INN = 1, nnp(eIdx)
			  !  IF (isbnd(1,indnd(INN,eIdx))==0 .AND. isbnd(2,indnd(INN,eIdx))==0) THEN ! internal node
			  !    IF (INDN_1==0) INDN_1 = indnd(INN,eIdx)
			  !    IF (INDN_1.GT.0 .AND. INDN_2==0) INDN_2 = indnd(INN,eIdx)
			!	ENDIF
			!  ENDDO
			  ! Then we do the changes to the boundary wave action spectrum
			 ! IF (INDN_1*INDN_2.GT.0) THEN
             !   CALL MEAN_WAVE_PARAMETER(IP,AC2(:,:,INDN_1),HS,ETOT,SME01,SME10,KME01,KMWAM,KMWAM2)
             !   IF (HS.GT.HSBC) THEN
             !     AC2(:,:,eIdx) = 0.5D0*(AC2(:,:,INDN_1)+AC2(:,:,INDN_2))
              !  ENDIF
             ! ENDIF				
		    ENDIF
          ENDIF
...
!**********************************************************************
      ...
      WRITE(16,*)'NUM_NETCDF_FILES_BND =,',NUM_NETCDF_FILES_BND
...
!**********************************************************************
      ...
      CALL SET_WAVE_BREAKING_BOUNDARY
      IF (LNANINFCHK) THEN
...
!**********************************************************************
      ...
      USE DATAPOOL, ONLY: NUMDIR_WW3, FR
# ifdef SCHISM
...
!          WRITE(STAT%FHNDL,*)'WW3 FMAX = ',FQ_WW3(NUMSIG_WW3),'WWM FMAX = ', FRHIGH
      ...
      ! KM
	  !WRITE(20,*)'FREQUENCIES'
      !WRITE(20,*)FR
      !WRITE(20,*)'DIRECTIONS'
      !WRITE(20,*)SPDIR
	  !DO IBWW3 = 1, NUMSIG
	  !	WRITE(20,*)'SPECTRUM f',IBWW3
      ! WRITE(20,*)SPEC_WWM(IBWW3,:,1)
	  !ENDDO
