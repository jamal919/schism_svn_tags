...
!* This only consists in reducing the energy at nodes in shallow areas
!**********************************************************************
      SUBROUTINE SET_WAVE_BREAKING_BOUNDARY
      USE DATAPOOL
      IMPLICIT NONE
      INTEGER :: IP, eIdx, IS, ID
      REAL(rkind) :: HS1, HS2, ETOT, SME01, SME10, KME01, KMWAM, KMWAM2
      ...
      IF (LBCWA .OR. LBCSP) THEN ! Spectrum or parametric boundary condition
        DO IP = 1, IWBMNP
          ...
          CALL MEAN_WAVE_PARAMETER(IP,AC2(:,:,eIdx),HS1,ETOT,SME01,SME10,KME01,KMWAM,KMWAM2)
		  BRHDLOC = HS1*(DEP(eIdx) + eta1(eIdx))
          ...
          ELSE		  
		    ...
		    ! We impose this reduction to the wave action
			IF (BETA.GT.1) THEN
			  CONTINUE
            ELSE
		    ...
		    ! Some check
            CALL MEAN_WAVE_PARAMETER(IP,AC2(:,:,eIdx),HS2,ETOT,SME01,SME10,KME01,KMWAM,KMWAM2)
          ENDIF
		ENDDO
      ENDIF
      END SUBROUTINE
