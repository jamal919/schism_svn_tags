...
! Will Perrie (Fisheries, Canada), Bash Toulany (Fisheries, Canada), Harry Wang (VIMS), Andre Fortunato (LNEC), Guillaume Dodet (LNEC), Kai Li (LNEC)
...
!**********************************************************************
 ...
 !!!     SUBROUTINE WWM_II(IT_SCHISM,icou_elfe_wwm,DT_SELFE0,NSTEP_WWM0)
      SUBROUTINE WWM_II(IT_SCHISM,icou_elfe_wwm,DT_SCHISM0,NSTEP_WWM0,RADFLAG2)
         ...
         INTEGER     :: I, IP, IT_SCHISM, K, IFILE, IT, INDCUT
         ...
         DO IP = 1, MNP
           ...
           IF (DEP(IP) .GT. DMIN) THEN
             ! Wave outputs, we apply a high frequency cutoff if provided in wwminput.nml
             IF (OUTFRHIGHFLAG) THEN
               INDCUT = MINLOC(ABS(SPSIG/PI2-OUTFRHIGH),1)
               IF (SPSIG(INDCUT)/PI2.LT.OUTFRHIGH) THEN
                 INDCUT = INDCUT + 1
               ENDIF
               CALL INTPAR(IP, INDCUT, ACLOC, OUTPAR)
             ELSE
