...
!**********************************************************************
      SUBROUTINE RADIATION_STRESS_SCHISM
        ...
        REAL(rkind)  :: dr_dxy(2,NVRT,MNS),HTOT,SXX3D0(NVRT,MNP),SYY3D0(NVRT,MNP),SXY3D0(NVRT,MNP)
        REAL(rkind)  :: WILD1(NVRT,MNP),WILD2(NVRT,MNP),WILD3(2,NVRT,MNS),WILD4(3,NVRT,MNP),DSPX,DSPY, WILD5(10)
...
!       and has a dimension of m/s/s
        call hgrad_nodes(IMET_DRY,0,NVRT,MNP,MNS,SXX3D0,dr_dxy)
...
!            if (isidenode(1,IS) == 150 .AND. isidenode(2,IS) == 149) stop
        ...
        call hgrad_nodes(IMET_DRY,0,NVRT,MNP,MNS,SYY3D0,dr_dxy)
        call exchange_s3d_2(dr_dxy)
        do IS=1,MNS
          if(idry_s(IS)==0) then
            HTOT=(eta2(isidenode(1,IS))+eta2(isidenode(2,IS))+DEP8(isidenode(1,IS))+DEP8(isidenode(2,IS)))/2
        ...
        call hgrad_nodes(IMET_DRY,0,NVRT,MNP,MNS,SXY3D0,dr_dxy)
        call exchange_s3d_2(dr_dxy)
!
...
!**********************************************************************
      SUBROUTINE COMPUTE_CONSERVATIVE_VF_TERMS_SCHISM
        USE DATAPOOL
        use schism_glbl, only: iplg,errmsg,hmin_radstress,kbp,kbs,kbe,ns,ne,nea,idry_s,idry_e, &
        ...
        real(rkind) :: stokes_w(NVRT,MNP), stokes_w_sd(NVRT,MNS), ws_tmp1(NVRT,MNS), ws_tmp2(NVRT,MNS)
...
!...    Contribution from the terms with Coriolis force and the spatial derivative of u
        call hgrad_nodes(2,0,NVRT,MNP,MNS,uu2,dr_dxy)
        do IS = 1,ns
          if(idry_s(IS) == 0) then
            do k = kbs(IS),NVRT
               ! f*v_s-du/dy*v_s
...
!...    Contribution from the terms with spatial derivative of v
        call hgrad_nodes(2,0,NVRT,MNP,MNS,vv2,dr_dxy)
        do IS = 1,ns
          if(idry_s(IS) == 0) then
            do k = kbs(IS),NVRT
               ! dv/dx*v_s
...
!...    Compute horizontal gradient of Stokes x and y-vel. (to compute Stokes z-vel.)
        ...
        call hgrad_nodes(2,0,NVRT,MNP,MNS,STOKES_VEL(1,:,:),dr_dxy)
        ws_tmp1(:,:) = dr_dxy(1,:,:)
        call hgrad_nodes(2,0,NVRT,MNP,MNS,STOKES_VEL(2,:,:),dr_dxy)
        ws_tmp2(:,:) = dr_dxy(2,:,:)

		
!...    Compute Stokes z-vel. at all levels: stokes_w_sd(NVRT,MNS)
        stokes_w_sd = 0.d0
...
!**********************************************************************
      SUBROUTINE COMPUTE_BREAKING_VF_TERMS_SCHISM
        USE DATAPOOL
        USE schism_glbl, only: iplg,errmsg,hmin_radstress,kbp,kbs,kbe,ns,np,ne,nea,idry_s,idry_e, &
...
!**********************************************************************
      SUBROUTINE SHORELINE_WAVE_FORCES
        ...
        USE schism_glbl, only: iplg,errmsg,hmin_radstress,kbp,wwave_force,idry,idry_s,idry_e, &
                             & isdel,elnode,i34,dldxy,grav,thetai,eta1,eta2,ns
        USE schism_msgp 
        IMPLICIT NONE
