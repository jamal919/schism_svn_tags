      ...
      SELECT CASE(IBREAK)
       ...
       CASE(2)
        IF (ICOMP .GE. 2) THEN
          IF ( BETA .GT. 0D0 ) THEN
            IF ( BETA .LT. 1D0 ) THEN
              WS = -(0.75d0*ALPBJ**3.d0*SME/MyREAL(SQRT(PI)))*BETA**4*SQRT(8.d0*ETOT)/DEP(IP)
        ...
        ELSE ! Only works in explicit for now
          IF ( BETA .GT. 0D0 ) THEN
            IF ( BETA .LT. 1D0 ) THEN
              SURFA0 = -(0.75d0*ALPBJ**3.d0*SME/MyREAL(SQRT(PI)))*BETA**6*SQRT(8.d0*ETOT)/DEP(IP) 
              SURFA1 = 0D0
            ELSE
      ...
      DO IS = 1, MSC
        DO ID = 1, MDC
          ...
          ELSE IF (ICOMP .LT. 2) THEN
            DSSBR(IS,ID)  = 0D0!SURFA0
            SSBR(IS,ID)   = SURFA0 * ACLOC(IS,ID)
            IMATDA(IS,ID) = IMATDA(IS,ID) + 0D0!SURFA0
            IMATRA(IS,ID) = IMATRA(IS,ID) + SSBR(IS,ID)
          END IF
        END DO
      END DO 
