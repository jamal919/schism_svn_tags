...
!**********************************************************************
      ...
      DO IS = 1, MSC
        ...
        DO ID = 1, MDC
          ...
          ELSE IF (ICOMP .LT. 2) THEN
            IMATDA(IS,ID) = IMATDA(IS,ID) + 0D0!DSSBF(IS,ID)
      ...
      DO IS=1,MSC
        DO ID=1,MDC
          !SBF(1,IP) = SBF(1,IP) + SINTH(ID)*(WK(IS,IP)/SPSIG(IS))*SSBF(IS,ID)*DS_INCR(IS)*DDIR
          !SBF(2,IP) = SBF(2,IP) + COSTH(ID)*(WK(IS,IP)/SPSIG(IS))*SSBF(IS,ID)*DS_INCR(IS)*DDIR
          SBF(1,IP) = SBF(1,IP) + COSTH(ID)*WK(IS,IP)*SSBF(IS,ID)*DS_INCR(IS)*DDIR !TG: SPSIG(IS) is simplified
          SBF(2,IP) = SBF(2,IP) + SINTH(ID)*WK(IS,IP)*SSBF(IS,ID)*DS_INCR(IS)*DDIR !TG: SPSIG(IS) is simplified
        ENDDO
      ENDDO
#endif
#ifdef DEBUG
