...
!     Authors: Kévin Martins, Xavier Bertin
!===============================================================================
      subroutine wbl_Soulsby97(Uc_x,Uc_y,z0,sigma,uorb,bthick,tau_bot)
!     Inputs:
...
!     Output: 
!             tau_bot - absolute value of the bottom stress (m^2/s/s)
...
!             can be done a priori.
      ...
      real(rkind), intent(out) :: tau_bot

      !Local
      real(rkind) :: rkappa, epsi, Uc, tau_c, fc, tau_w, fw
      ...
      Uc = sqrt(Uc_x**2d0 + Uc_y**2d0)             ! Norm of the depth-averaged current velocity
      fc = (rkappa/(1d0 + dlog(z0/bthick)))**2d0   ! Current related friction coefficient
      tau_c = fc*Uc*Uc                             ! Norm of the the current-induced shear stress (skin friction)
      ...
      tau_bot = tau_c*(1+1.2*(tau_w/max(epsi,tau_w+tau_c)))
