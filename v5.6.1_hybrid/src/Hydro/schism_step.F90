...
!                      BASIC_PARAMETER() in wwm_initio.F90 for details; below is a snapshot from there:
         ...
         !OUTPAR(7)   = ETOTC    ! Etot energy in y-direction
         !OUTPAR(8)   = ETOTS    ! Etot energy in x-direction
...
!         Wet node
          ...
          if(rough_p(i)<=0) then !time-independent Cd
            ...
            if(bthick<=rough_p(i)) then
              ...
              !WBL
#ifdef USE_WWM
              ! Wave and current quantities used in both formulations for the WBL
              ...
              ! Wave boundary layer
              if(iwbl == 1) then ! Grant and Madsen type
                ...
                taub_out(i) = sqrt(Cdp(i))*(uu2(kbp(i)+1,i)**2+vv2(kbp(i)+1,i)**2)
              elseif(iwbl == 2) then! Soulsby (1997) type
                call wbl_Soulsby97(uu2(kbp(i)+1,i),vv2(kbp(i)+1,i),rough_p(i),wfr,ubm,bthick,taub)
                taub_out(i) = taub
              endif !iwbl             
#endif /*USE_WWM*/
            endif !bthick
...
! KM
        ! Bottom stress
        !call writeout_nc(id_out_var(200),'bottom_stress',1,1,npa,dble(taub_out))
! KM
