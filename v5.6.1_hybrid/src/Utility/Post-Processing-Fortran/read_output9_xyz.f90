...
!       and calculate time series for 3D variables (surface values for 2D variables), DEFINED AT NODES OR
!       ELEMENTS.
...
!              (4) combined nc outputs (schout*.nc; tri-quad)
!       Outputs: fort.1[89]; ; fort.20 - local depth for each pt.
!       For ics=2 (e.g. for lon/lat), use nearest node for output
!											
!   ifort -mcmodel=medium -assume byterecl -O2 -o read_output9_xyz.WW ../UtilLib/extract_mod.f90 read_output9_xyz.f90 ../UtilLib/compute_zcor.f90 ../UtilLib/pt_in_poly.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
!****************************************************************************************
!
      program read_out
      use netcdf
...
!...  Header
      ...
      print*, 'After header:',ne,np,ns,nrec,i34(ne),itime_id,ielev_id, &
     &elnode(1:i34(ne),ne),nvrt,h0,x(np),y(np),dp(np),start_time

!     Read in vgrid.in 
      allocate(timeout(nrec),ztot(nvrt),sigma(nvrt),sigma_lcl(nvrt,np), &
     &kbp(np),outvar(2,nvrt,max(np,ne,ns)),eta2(np),out2(nxy,nvrt,2),out3(nxy,2), &
...
!'
                ...
                do m=1,ivs
                  if(i23d<=3) then !node
                    ...
                    out3(i,m)=outvar(m,k0+1,iep(i)) !FV
                  endif
                enddo !m
              endif
            endif !dry/wet
