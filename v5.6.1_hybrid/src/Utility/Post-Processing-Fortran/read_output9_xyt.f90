...
!									
! ifort -mcmodel=medium -assume byterecl -O2 -o read_output9_xyt.WW ../UtilLib/extract_mod.f90 read_output9_xyt.f90 ../UtilLib/compute_zcor.f90 ../UtilLib/pt_in_poly.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
!****************************************************************************************
!
      program read_out
      use netcdf
...
!     Read in vgrid.in
      allocate(timeout(nrec),ztot(nvrt),sigma(nvrt),sigma_lcl(nvrt,np),kbp(np),outvar(2,nvrt,max(np,ne,ns)), &
            ...
            do j=1,3 !nodes
              ...
              do m=1,ivs
                if(i23d<=3) then !node
                  ...
                  if(iep(i)>0) out2(l,1,m)=outvar(m,1,iep(i))
                endif
              enddo !m
            enddo !j
          else !3D
                ...
                do m=1,ivs
                  do j=1,3
                    ...
                    if(i23d<=3) then !node
                      ...
                      if(iep(i)>0) out2(l,k,m)=outvar(m,k,iep(i))
                    endif !i23d
                  enddo !j
                enddo !m
              enddo !k
