...
! ifort  -O2 -mcmodel=medium -assume byterecl -O2 -o read_output9_transect.WW ../UtilLib/extract_mod.f90 read_output9_transect.f90 moving_average_filter.f90 ../UtilLib/compute_zcor.f90 ../UtilLib/pt_in_poly.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -L$NETCDF/lib -lnetcdf -lnetcdff
!											*
!****************************************************************************************
!
      program read_out
...
!     Read in vgrid.in
     ...
     &timeout(nrec),outvar(2,nvrt,max(np,ne,ns)))
...
!                  write(12,*)'Warning: failed to find a vertical level:',it,i
                  ...
                  do m=1,ivs
                    if(i23d<=3) then !node based
                      ...
                      out4(kk,m)=outvar(m,k0+1,iep(i)) !FV
                    endif
